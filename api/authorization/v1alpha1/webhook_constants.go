...
// roleTargetCollision reports whether two role targets collide after the caller
// has already matched candidates by spec.targetName. ClusterRole targets always
// collide (cluster-scoped names are unique); Role targets only collide when
// targetNamespace also matches.
