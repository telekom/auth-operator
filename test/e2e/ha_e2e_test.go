...
var _ = Describe("Leader Election and HA E2E", Ordered, Label("ha", "leader-election"), func() {
	...
	BeforeAll(func() {
		...
		for _, ns := range []string{haNamespace, haTestNamespace} {
			cmd := utils.CommandContext(context.Background(), "kubectl", "create", "ns", ns, "--dry-run=client", "-o", "yaml") // #nosec G204
		...
		cmd := utils.CommandContext(context.Background(), "kubectl", "label", "ns", haTestNamespace, "e2e-ha-test=true", "--overwrite") // #nosec G204
		_, _ = utils.Run(cmd)
	...
	AfterAll(func() {
		...
		cmd := utils.CommandContext(context.Background(), "helm", "uninstall", haHelmRelease, "-n", haNamespace, "--wait", "--timeout", "2m") // #nosec G204
		_, _ = utils.Run(cmd)

		By("Cleaning up namespaces")
		for _, ns := range []string{haNamespace, haTestNamespace} {
			cmd := utils.CommandContext(context.Background(), "kubectl", "delete", "ns", ns, "--ignore-not-found=true") // #nosec G204
			_, _ = utils.Run(cmd)
		}
	...
	Context("Multiple Controller Replicas with Leader Election", func() {
		It("should deploy with multiple controller replicas", func() {
			...
			cmd := utils.CommandContext(context.Background(), "helm", "install", haHelmRelease, helmChartPath, // #nosec G204
				"-n", haNamespace,
				"--create-namespace",
		...
		It("should have exactly one leader", func() {
			...
			Eventually(func() bool {
				cmd := utils.CommandContext(context.Background(), "kubectl", "get", "lease", // #nosec G204
					"-n", haNamespace,
			...
			cmd := utils.CommandContext(context.Background(), "kubectl", "logs", // #nosec G204
				"-l", "control-plane=controller-manager",
				"-n", haNamespace,
				"--tail=100")
			output, _ := utils.Run(cmd)
		...
		It("should have PodDisruptionBudgets configured correctly", func() {
			...
			Eventually(func() bool {
				cmd := utils.CommandContext(context.Background(), "kubectl", "get", "pdb", // #nosec G204
					"-l", "control-plane=controller-manager",
					"-n", haNamespace,
...
`
		...
		It("should elect a new leader when current leader is terminated", func() {
			...
			if len(leaderParts) > 0 {
				...
				cmd := utils.CommandContext(context.Background(), "kubectl", "delete", "pod", leaderPod, // #nosec G204
...
`, i, i)
			...
			Eventually(func() int {
				cmd := utils.CommandContext(context.Background(), "kubectl", "get", "webhookauthorizer", "-o", "name") // #nosec G204
				output, err := utils.Run(cmd)
				if err != nil {
					return 0
				}
	...
	Context("Resource Scaling", func() {
		It("should scale down webhook replicas", func() {
			...
			cmd := utils.CommandContext(context.Background(), "kubectl", "scale", "deployment", // #nosec G204
				"-l", "control-plane=webhook-server",
				"-n", haNamespace,
				"--replicas=1")
			_, err := utils.Run(cmd)
...
func dumpAllPodLogs(namespace, filename string) {
	cmd := utils.CommandContext(context.Background(), "kubectl", "get", "pods", "-n", namespace, "-o", "name") // #nosec G204
	output, err := utils.Run(cmd)
	if err != nil {
		return
	}
	...
	for _, podName := range utils.GetNonEmptyLines(string(output)) {
		fmt.Fprintf(&allLogs, "\n=== Logs for %s ===\n", podName)
		cmd := utils.CommandContext(context.Background(), "kubectl", "logs", podName, "-n", namespace, "--tail=200") // #nosec G204
...
func createHASummary(namespace, timestamp string) {
	...
	cmd := utils.CommandContext(context.Background(), "kubectl", "get", "pods", "-n", namespace, "-o", "wide") // #nosec G204
	output, _ := utils.Run(cmd)
	summary.WriteString("```\n")
	...
	cmd = utils.CommandContext(context.Background(), "kubectl", "get", "lease", "-n", namespace, "-o", "wide") // #nosec G204
	output, _ = utils.Run(cmd)
	summary.WriteString("```\n")
	summary.WriteString(string(output))
	summary.WriteString("```\n\n")
	...
	cmd = utils.CommandContext(context.Background(), "kubectl", "get", "pdb", "-n", namespace, "-o", "wide") // #nosec G204
	output, _ = utils.Run(cmd)
	summary.WriteString("```\n")
	summary.WriteString(string(output))
	summary.WriteString("```\n")
