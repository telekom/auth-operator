...
var _ = Describe("Resilience - Webhook Failure Injection", Ordered, Label("complex", "resilience"), func() {
	...
	Context("TLS Certificate Rotation Recovery", func() {
		...
		It("should recover after TLS cert invalidation triggers rotation", func() {
			...
			Expect(err).NotTo(HaveOccurred(), "Failed to list webhook TLS secrets")
			tlsSecretNames := strings.Fields(strings.TrimSpace(string(output)))
			if len(tlsSecretNames) == 0 {
				Skip("No TLS secret found — skipping TLS rotation test")
			}
			Expect(tlsSecretNames).To(HaveLen(1), "expected exactly one webhook TLS secret")
			tlsSecretName := tlsSecretNames[0]
			...
			Eventually(func() bool {
				...
				if err != nil || len(strings.TrimSpace(string(tlsCrt))) == 0 {
					return false
				}
				...
				return len(strings.TrimSpace(string(tlsKey))) > 0
			}, whDeployTimeout, whPollInterval).Should(BeTrue(),
				"TLS secret should contain regenerated tls.crt and tls.key data after rotation")
...
`, ssaExternalRDName, ssaExternalCRName)
		...
		It("should self-heal when an external actor modifies the SSA-managed ClusterRole", func() {
			...
			Eventually(func() (bool, error) {
				...
				return strings.Contains(rulesStr, "verbs"), nil
