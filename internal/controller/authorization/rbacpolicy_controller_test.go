...
func rbacPolicyCtx(t *testing.T) context.Context {
	t.Helper()
	return ctrllog.IntoContext(t.Context(), logr.Discard())
}
...
func TestRBACPolicy_Reconcile_NotFound(t *testing.T) {
	...
	result, err := r.Reconcile(rbacPolicyCtx(t), rbacPolicyRequest("nonexistent"))
...
func TestRBACPolicy_Reconcile_NoBoundResources(t *testing.T) {
	...
	result, err := r.Reconcile(rbacPolicyCtx(t), rbacPolicyRequest("test-policy"))
	g.Expect(err).NotTo(gomega.HaveOccurred())
	g.Expect(result).To(gomega.Equal(ctrl.Result{}))

	var updated authorizationv1alpha1.RBACPolicy
	g.Expect(c.Get(rbacPolicyCtx(t), types.NamespacedName{Name: "test-policy"}, &updated)).To(gomega.Succeed())
...
func TestRBACPolicy_Reconcile_WithBoundResources(t *testing.T) {
	...
	result, err := r.Reconcile(rbacPolicyCtx(t), rbacPolicyRequest("team-policy"))
	g.Expect(err).NotTo(gomega.HaveOccurred())
	g.Expect(result).To(gomega.Equal(ctrl.Result{}))

	var updated authorizationv1alpha1.RBACPolicy
	g.Expect(c.Get(rbacPolicyCtx(t), types.NamespacedName{Name: "team-policy"}, &updated)).To(gomega.Succeed())
...
func TestRBACPolicy_Reconcile_ObservesGeneration(t *testing.T) {
	...
	r, c := newRBACPolicyTestReconciler(pol)
	_, err := r.Reconcile(rbacPolicyCtx(t), rbacPolicyRequest("gen-policy"))
	g.Expect(err).NotTo(gomega.HaveOccurred())

	var updated authorizationv1alpha1.RBACPolicy
	g.Expect(c.Get(rbacPolicyCtx(t), types.NamespacedName{Name: "gen-policy"}, &updated)).To(gomega.Succeed())
...
func TestRBACPolicy_Reconcile_GetError(t *testing.T) {
	...
	_, err := r.Reconcile(rbacPolicyCtx(t), rbacPolicyRequest("any-policy"))
...
func TestRBACPolicy_Reconcile_ListError(t *testing.T) {
	...
	_, err := r.Reconcile(rbacPolicyCtx(t), rbacPolicyRequest("list-error-policy"))
	g.Expect(err).To(gomega.HaveOccurred())
...
func TestRBACPolicy_RestrictedResourceToPolicyRequests(t *testing.T) {
	...
	requests := r.restrictedResourceToPolicyRequests(rbacPolicyCtx(t), rbd)
	...
	requests = r.restrictedResourceToPolicyRequests(rbacPolicyCtx(t), rrd)
	g.Expect(requests).To(gomega.HaveLen(1))
	g.Expect(requests[0].Name).To(gomega.Equal("other-policy"))

	// Unknown type returns nil.
	requests = r.restrictedResourceToPolicyRequests(rbacPolicyCtx(t), &authorizationv1alpha1.RoleDefinition{})
	g.Expect(requests).To(gomega.BeNil())

	// Empty policyRef returns nil.
	rbd2 := &authorizationv1alpha1.RestrictedBindDefinition{}
	requests = r.restrictedResourceToPolicyRequests(rbacPolicyCtx(t), rbd2)
	g.Expect(requests).To(gomega.BeNil())
}
...
func TestRBACPolicy_MarkStalled(t *testing.T) {
	...
	r, c := newRBACPolicyTestReconciler(pol)
	r.markStalled(rbacPolicyCtx(t), pol, fmt.Errorf("something failed"))

	var updated authorizationv1alpha1.RBACPolicy
	g.Expect(c.Get(rbacPolicyCtx(t), types.NamespacedName{Name: "stalled-policy"}, &updated)).To(gomega.Succeed())
...
func TestRBACPolicy_Reconcile_WithTracer(t *testing.T) {
	...
	result, err := r.Reconcile(rbacPolicyCtx(t), ctrl.Request{
...
func TestRBACPolicy_BoundResourceCount_NonNegative(t *testing.T) {
	g := gomega.NewWithT(t)
	...
	r, c := newRBACPolicyTestReconciler(pol)
	_, err := r.Reconcile(rbacPolicyCtx(t), rbacPolicyRequest("count-policy"))
	g.Expect(err).NotTo(gomega.HaveOccurred())

	var updated authorizationv1alpha1.RBACPolicy
	g.Expect(c.Get(rbacPolicyCtx(t), types.NamespacedName{Name: "count-policy"}, &updated)).To(gomega.Succeed())
	g.Expect(updated.Status.BoundResourceCount).To(gomega.BeNumerically(">=", int32(0)))
}
...
func TestRBACPolicy_Reconcile_WithTracer_Error(t *testing.T) {
	...
	_, err := r.Reconcile(rbacPolicyCtx(t), ctrl.Request{
