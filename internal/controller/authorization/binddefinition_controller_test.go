...
func TestReconcileDeleteCleansUpGaugeMetrics(t *testing.T) {
	...
	metrics.ServiceAccountOwnershipTakeovers.WithLabelValues(bdName).Inc()
...
func expectBindDefinitionMetricSeriesCleared(g *WithT, name string) {
	...
	g.Expect(testutil.ToFloat64(metrics.ServiceAccountOwnershipTakeovers.WithLabelValues(name))).To(Equal(float64(0)))
