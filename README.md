...
Reading at the naming convention in the above format may seem very abstract. However lets take some example input values and construct groups names. The example values are the following:
...
Some other groups may not have this necessity - namely the platform team, the onboarding team, third party teams and the first line support team. These teams will only have `Global` mappings as their teams provide support to multiple cluster consumers across numerous clusters. It would be extremely unpractical and tedious to juggle membership per-cluster for a platform team which **must** support all clusters. The same is true for onboarding, thid party teams and first line support - as they gain access to a cluster by toggling the participant in the `TCaasTenant` specification or by providing overrides in the `TCaasCluster`. If you are unaware of this topic please refer to the chapter above this one. Otherwise proceed to role mappings and their respective permissions in the next chapter.

___

## Role mappings
Role mappings are a way to uniquely identify the permission scope and may very depending on the cluster participant. For some role mappings you will see a total of 7 roles, but 2 of these roles are assigned through the same group-role mapping. The roles in question are `*-namespaced-reader` and `*-namespaced-reader-restricted`. These roles are assignable through group suffixed with `*-reader` because they are related to cluster participants of type `owners` where the `*-namespaced-reader` is a direct mapping to `owning` namespaces and `*-namespaced-reader-restricted` is a direct mapping to `non-owning` namespaces. These role mappings are subject to change and could be changed if new security circumstances arise.

#### Platform team
| Role | Description | Allowed verbs | Restricted API groups | Restricted API resources |
| ---- | ----------- | ------------- | --------------------- | ------------------------ |
| Platform Cluster Poweruser | Can execute all verbs on cluster scoped resources | create, delete, deletecollection, patch, update, get, list, watch | | |
| Platform Cluster Collaborator | Can execute edit verbs on cluster scoped resources | patch, update, get, list, watch	 | | |
| Platform Cluster Reader | Can execute read verbs on cluster scoped resources | get, list, watch | | |
| Platform Namespaced Poweruser | Can execute all verbs on namespace scoped resources in owning namespaces | create, delete, deletecollection, patch, update, get, list, watch | | |
| Platform Namespaced Collaborator | Can execute edit verbs on namespace scoped resources in owning namespaces | patch, update, get, list, watch | | |
| Platform Namespaced Reader | Can execute read verbs on namespace scoped resources in owning namespaces | get, list, watch | | |
| Platform Namespaced Reader Restricted | Can execute read verbs on namespaced scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources. This role is attached to tenant namespaces and third party namespaces. | get, list, watch | | secrets, pods/exec, pods/proxy, pods/attach, pods/portforward |


#### Tenant
| Role | Description | Allowed verbs | Restricted API groups | Restricted API resources |
| ---- | ----------- | ------------- | --------------------- | ------------------------ |
| Tenant Cluster Poweruser | Can execute all verbs on cluster scoped resources, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, crd.projectcalico.org/v1, node.k8s.io/v1, trident.netapp.io/v1 | nodes, nodes/proxy |
| Tenant Cluster Collaborator | Can execute edit verbs on cluster scoped resources, except on restricted API groups and restricted API resources | patch, update, get, list, watch | authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, crd.projectcalico.org/v1, node.k8s.io/v1, trident.netapp.io/v1 | nodes, nodes/proxy |
| Tenant Cluster Reader | Can execute read verbs on cluster scoped resources, except on restricted API groups and restricted API resources | get, list, watch | | nodes/proxy |
| Tenant Namespaced Poweruser | Can execute all verbs on namespace scoped resources in owning namespaces, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | crd.projectcalico.org/v1, trident.netapp.io/v1 | |
| Tenant Namespaced Collaborator | Can execute edit verbs on namespace scoped resources in owning namespaces, except on restricted API groups and restricted API resources | patch, update, get, list, watch | crd.projectcalico.org/v1, trident.netapp.io/v1 | |
| Tenant Namespaced Reader | Can execute read verbs on namespace scoped resources in owning namespaces, except on restricted API groups and restricted API resources | get, list, watch | | |
| Tenant Namespaced Reader Restricted | Can execute read verbs on namespaced scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources. This role is attached to platform namespaces and third party namespaces. | get, list, watch | | secrets, pods/attach, pods/binding, pods/ephemeralcontainers, pods/eviction,  pods/exec, pods/log, pods/portforward, pods/proxy, serviceaccounts/token, services/proxy |

#### Third parties
| Role | Description | Allowed verbs | Restricted API groups | Restricted API resources |
| ---- | ----------- | ------------- | --------------------- | ------------------------ |
| Third party Cluster Poweruser | Can execute all verbs on cluster scoped resources, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, crd.projectcalico.org/v1, node.k8s.io/v1, trident.netapp.io/v1 | nodes, nodes/proxy |
| Third party Cluster Collaborator | Can execute edit verbs on cluster scoped resources, except on restricted API groups and restricted API resources | patch, update, get, list, watch | authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, crd.projectcalico.org/v1, node.k8s.io/v1, trident.netapp.io/v1 | nodes, nodes/proxy |
| Third party Cluster Reader | Can execute read verbs on cluster scoped resources, except on restricted API groups and restricted API resources | get, list, watch | | nodes/proxy |
| Third party Namespaced Poweruser | Can execute all verbs on namespace scoped resources in owning namespaces, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | crd.projectcalico.org/v1, trident.netapp.io/v1 | |
| Third party Namespaced Collaborator | Can execute edit verbs on namespace scoped resources in owning namespaces, except on restricted API groups and restricted API resources | patch, update, get, list, watch | crd.projectcalico.org/v1, trident.netapp.io/v1 | |
| Third party Namespaced Reader | Can execute read verbs on namespace scoped resources in owning namespaces, except on restricted API groups and restricted API resources | get, list, watch | | |
| Third party Namespaced Reader Restricted | Can execute read verbs on namespaced scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources. This role is attached to platform namespaces and tenant namespaces. | get, list, watch | | secrets, pods/attach, pods/binding, pods/ephemeralcontainers, pods/eviction,  pods/exec, pods/log, pods/portforward, pods/proxy, serviceaccounts/token, services/proxy |
...
| Role | Description | Allowed verbs | Restricted API groups | Restricted API resources |
| ---- | ----------- | ------------- | --------------------- | ------------------------ |
| Onboarding Cluster Poweruser | Can execute all verbs on cluster scoped resources, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | admissionregistration.k8s.io/v1, apiextensions.k8s.io/v1, apiregistration.k8s.io/v1, authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, certificates.k8s.io/v1, crd.projectcalico.org/v1, flowcontrol.apiserver.k8s.io/v1, node.k8s.io/v1, rbac.authorization.k8s.io/v1, scheduling.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, trident.netapp.io/v1 | namespaces, namespaces/finalize, nodes, nodes/proxy |
| Onboarding Cluster Collaborator | Can execute edit verbs on cluster scoped resources, except on restricted API groups and restricted API resources | patch, update, get, list, watch | admissionregistration.k8s.io/v1, apiextensions.k8s.io/v1, apiregistration.k8s.io/v1, authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, certificates.k8s.io/v1, crd.projectcalico.org/v1, flowcontrol.apiserver.k8s.io/v1, node.k8s.io/v1, rbac.authorization.k8s.io/v1, scheduling.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, trident.netapp.io/v1 | namespaces, namespaces/finalize, nodes, nodes/proxy |
| Onboarding Cluster Reader | Can execute read verbs on cluster scoped resources, except on restricted API groups and restricted API resources | get, list, watch | | nodes/proxy |
| Onboarding Namespaced Poweruser | Can execute all verbs on namespace scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | acme.cert-manager.io/v1, autoscaling/v2, batch/v1, cert-manager.io/v1, coordination.k8s.io/v1, crd.projectcalico.org/v1, events.k8s.io/v1, policy/v1, rbac.authorization.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, velero.io/v1, trident.netapp.io/v1 | secrets, pods/attach, pods/ephemeralcontainers, pods/exec, pods/portforward, pods/proxy |
| Onboarding Namespaced Collaborator | Can execute edit verbs on namespace scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources | patch, update, get, list, watch | acme.cert-manager.io/v1, autoscaling/v2, batch/v1, cert-manager.io/v1, coordination.k8s.io/v1, crd.projectcalico.org/v1, events.k8s.io/v1, policy/v1, rbac.authorization.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, velero.io/v1, trident.netapp.io/v1 | secrets, pods/attach, pods/ephemeralcontainers, pods/exec, pods/portforward, pods/proxy |
| Onboarding Namespaced Reader | Can execute read verbs on namespace scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources | get, list, watch | | secrets, pods/attach, pods/ephemeralcontainers, pods/exec, pods/portforward, pods/proxy |
...
| Role | Description | Allowed verbs | Restricted API groups | Restricted API resources |
| ---- | ----------- | ------------- | --------------------- | ------------------------ |
| First line Cluster Poweruser | Can execute all verbs on cluster scoped resources, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | admissionregistration.k8s.io/v1, apiextensions.k8s.io/v1, apiregistration.k8s.io/v1, authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, certificates.k8s.io/v1, crd.projectcalico.org/v1, flowcontrol.apiserver.k8s.io/v1, node.k8s.io/v1, rbac.authorization.k8s.io/v1, scheduling.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, trident.netapp.io/v1 | namespaces, namespaces/finalize, nodes, nodes/proxy |
| First line Cluster Collaborator | Can execute edit verbs on cluster scoped resources, except on restricted API groups and restricted API resources | patch, update, get, list, watch | admissionregistration.k8s.io/v1, apiextensions.k8s.io/v1, apiregistration.k8s.io/v1, authentication.t-caas.telekom.com/v1alpha1, authorization.t-caas.telekom.com/v1alpha1, cert-manager.io/v1, certificates.k8s.io/v1, crd.projectcalico.org/v1, flowcontrol.apiserver.k8s.io/v1, node.k8s.io/v1, rbac.authorization.k8s.io/v1, scheduling.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, trident.netapp.io/v1 | namespaces, namespaces/finalize, nodes, nodes/proxy |
| First line Cluster Reader | Can execute read verbs on cluster scoped resources, except on restricted API groups and restricted API resources | get, list, watch | | nodes/proxy |
| First line Namespaced Poweruser | Can execute all verbs on namespace scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources | create, delete, deletecollection, patch, update, get, list, watch | acme.cert-manager.io/v1, autoscaling/v2, batch/v1, cert-manager.io/v1, coordination.k8s.io/v1, crd.projectcalico.org/v1, events.k8s.io/v1, policy/v1, rbac.authorization.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, velero.io/v1, trident.netapp.io/v1 | secrets, pods/attach, pods/binding, pods/ephemeralcontainers, pods/eviction,  pods/exec, pods/log, pods/portforward, pods/proxy, serviceaccounts/token, services/proxy |
| First line Namespaced Collaborator | Can execute edit verbs on namespace scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources | patch, update, get, list, watch | acme.cert-manager.io/v1, autoscaling/v2, batch/v1, cert-manager.io/v1, coordination.k8s.io/v1, crd.projectcalico.org/v1, events.k8s.io/v1, policy/v1, rbac.authorization.k8s.io/v1, snapshot.storage.k8s.io/v1, storage.k8s.io/v1, velero.io/v1, trident.netapp.io/v1 | secrets, pods/attach, pods/binding, pods/ephemeralcontainers, pods/eviction,  pods/exec, pods/log, pods/portforward, pods/proxy, serviceaccounts/token, services/proxy |
| First line Namespaced Reader | Can execute read verbs on namespace scoped resources in non-owning namespaces, except on restricted API groups and restricted API resources | get, list, watch | | secrets, pods/attach, pods/binding, pods/ephemeralcontainers, pods/eviction,  pods/exec, pods/log, pods/portforward, pods/proxy, serviceaccounts/token, services/proxy |
