...
spec:
  ...
  impersonation:
    ...
    onImpersonationFailure: Deny  # or: AllowWithAudit, CreateSA
```
...
```go
func (v *RestrictedBindDefinitionValidator) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
...
> **Note:** Because escalation prevention requires access to the requesting
> user's identity (`UserInfo`), this validating webhook is implemented as an
> `admission.Handler` rather than `admission.Validator`. The `admission.Validator`
> interface does not expose `admission.Request` or `UserInfo`.

```go
// RestrictedBindDefinitionWebhook implements admission.Handler to access
// the admission.Request (including UserInfo for escalation checks).
type RestrictedBindDefinitionWebhook struct {
    Client  client.Client
    decoder admission.Decoder
}

func (v *RestrictedBindDefinitionWebhook) Handle(ctx context.Context, req admission.Request) admission.Response {
    rbd := &authorizationv1alpha1.RestrictedBindDefinition{}
    if err := v.decoder.Decode(req, rbd); err != nil {
        return admission.Errored(http.StatusBadRequest, err)
    }
    
    // 1. Read explicit policy reference from the RestrictedBindDefinition
    if rbd.Spec.RBACPolicyRef.Name == "" {
        return admission.Denied("rbacPolicyRef.name is required")
    }
    ...
    if err := v.Client.Get(ctx, client.ObjectKey{Name: rbd.Spec.RBACPolicyRef.Name}, policy); err != nil {
        return admission.Denied(fmt.Sprintf("referenced RBACPolicy %q not found: %v", rbd.Spec.RBACPolicyRef.Name, err))
    }
    
    // 3. Verify the policy applies to this namespace (appliesTo check)
    if !v.policyAppliesToNamespace(policy, rbd.Namespace) {
        return admission.Denied(fmt.Sprintf("RBACPolicy %q does not apply to namespace %q", policy.Name, rbd.Namespace))
    }
    
    // 4. Validate against policy limits
    ...
    // 5. RBAC escalation check (user must have permissions they're granting)
    //    UserInfo is available directly from req.UserInfo
    if policy.Spec.EscalationPrevention.EnforceRBACEscalationPrevention {
        if err := v.checkEscalation(ctx, rbd, &req.UserInfo); err != nil {
            return admission.Denied(err.Error())
        }
    }
...
```
Evaluation:
  # Step 1: Forbidden selectors (any match → reject)
  ...
  # Step 4: Allowed selectors are defined but none matched
  REJECT
```
