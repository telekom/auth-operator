...
grep -Fq 'lock_with_flock()' "$runner"
grep -Fq 'flock or perl is required' "$runner"
grep -Fq 'start_session()' "$runner"
grep -Fq 'setsid or perl is required' "$runner"
...
grep -Fq 'canonical_child_path "$requested_results_dir" false' "$runner"
grep -Fq 'validate_owned_tree "$results_dir" true' "$runner"
...
grep -Fq 'start_session "$timeout_bin"' "$runner"
